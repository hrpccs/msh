...
MAIN_DIR=../../
BUILD_DIR=${MAIN_DIR}/apps/build
SCAVLIST=${MAIN_DIR}/scav.txt
RES_DIR=../results
TMP_DIR=../results/tmp
SCHED_NAME=template
TARGET=$(cat ${SCAVLIST} | awk '{print $1}')

echo "Capture deliquent load PCs ... "
echo "Target object: "
#echo -n "$TARGET_ABS_PATH " > $SCAVLIST
#echo $ARGS >> $SCAVLIST
cat $SCAVLIST
...
EVENT1=cpu_core/mem_load_retired.l2_miss/
EVENT2=cpu_core/mem_load_retired.l3_miss/
EVENT3=BR_INST_RETIRED.ALL_BRANCHES
sudo perf record -e ${EVENT1},${EVENT2},${EVENT3}\
        -b \
        -c${SAMPLING_RATE_IN_CNTS} \
        -o ${RES_DIR}/perf.data -- sudo sh -c ". ${MAIN_DIR}/config.sh; chrt -r 99 ${BUILD_DIR}/${SCHED_NAME} 1"

sudo chown ${USER} ${RES_DIR}/perf.data

#perf record -e cpu/event=0xd1,umask=0x10,name=mem_load_retired.l2_miss/ppp,cpu/event=0xd1,umask=0x20,name=mem_load_retired.l3_miss/ppp,cycles:u -b -F1000 -o ${RES_DIR}/perf.data -- ${BUILD_DIR}/${SCHED_NAME} ${SCAVLIST}
#perf record -e cpu/event=0xd1,umask=0x20,name=mem_load_retired.l3_miss/ppp -F1000 -o ${RES_DIR}/perf.data -- ${BUILD_DIR}/${SCHED_NAME} ${SCAVLIST}
...
python3 read_func.py ${TMP_DIR}/perf_report.txt ${TMP_DIR}/fn_list.txt ${SCHED_NAME}
...
for FUNC in $(cat ${TMP_DIR}/fn_list.txt | awk -F'[\t]' '{print $1}'); do
  ...
  for PC in $(cat ${TMP_DIR}/fn_${FUNC_NO_SPACE}_PClist.txt); do
    echo -n "${TARGET} " >> ${RES_DIR}/all_PClist.txt
